...
| Storage          | Hardening (Verify and Backup live) | Guards the learned data against corruption. A recent backup is always kept. If a power outage ever corrupts the integration's data file, damage is repaired where it can be, and the backup is restored where it cannot. |
