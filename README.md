...
| WiFi             | Experimental                   | Detects a WiFi outage by listening for your network from the Home Assistant server's own wireless adapter, so it works whatever router you own. Devices that went down with the network are counted rather than blamed one by one. Proven on one house so far, through deliberate outages and a real one, which is why it is marked experimental. A wireless adapter on your server is required; see the [WiFi](https://github.com/TheThinkingHome/device_sentinel/wiki/WiFi) documentation. |
